...
var (
	...
	revision  = "unknown" // sha1 from git, output of $(git rev-parse --short HEAD)
	buildDate = "unknown" // build date in ISO8601 format, output of $(date -u +'%Y-%m-%dT%H:%M:%SZ')
)

// Info holds the runtime version information.
type Info struct {
...
var versionInfoTmpl = `version:          {{.version}}
revision:         {{.revision}}
...
// Print returns a formatted, human-readable version string.
func Print() string {
	m := map[string]string{
		"version":   version,
		"revision":  revision,
